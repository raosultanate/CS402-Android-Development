...
class UserProfileActivity : BaseActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (userProfileData != null) {
            ...
            if (userProfileData.profileCompleted == 0) {

                completeProfileTextView.text = resources.getString(R.string.title_complete_Profile)
                ...
                completeProfileTextView.text = resources.getString(R.string.title_edit_profile)
                GlideLoader(this).loadUserPicture(userProfileData.image, userPhoto)

                completeProfile_EmailID_TextView.isEnabled = false
                completeProfile_EmailID_TextView.setText(userProfileData.email.toString())

                if(userProfileData.mobile != 0L){
                    completeProfile_MobileNumber_TextView.setText(userProfileData.mobile.toString())
                }
                if (userProfileData.gender == Constants.MALE) {
            ...
            if (requestCode == Constants.IMAGE_REQUEST_CODE) {
                if (data != null) {
                    try {
                        ...
                        Toast.makeText(
                            this,
                            resources.getString(R.string.imageLoad_Failed),
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
    ...
    private fun validateUserProfileDetails(): Boolean {
        return when {
            TextUtils.isEmpty(
                completeProfile_MobileNumber_TextView.text.toString().trim { it <= ' ' }) -> {
    ...
    fun userProfileUpdateSuccess() {
        Toast.makeText(this, resources.getString(R.string.update_success), Toast.LENGTH_SHORT)
            .show()
    ...
    private fun updateUserProfileDetails() {
        ...
        val firstName = completeProfile_FirstName_TextView.text.toString().trim { it <= ' ' }
        val lastName = completeProfile_LastName_TextView.text.toString().trim { it <= ' ' }

        //This means we are in the edit profile.
        ...
        if(gender.isNotEmpty() && gender != userProfileData.gender){
        ...
        if (mobileNumber.isNotEmpty() && mobileNumber != userProfileData.mobile.toString()) {
